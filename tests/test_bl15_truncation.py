...
# ═══ 🚨 불평·질문이 명령으로 실행되던 것 (2026-09-09 실기 사고) ═══
#
# *"아니 설정 창은 애초에 실행 중이지 않았는데 뭘 가져와 너 왜 나한테 뻥카 치냐"*
# 가 `설정 열어줘`에 매칭돼 **open_app이 실제로 실행됐다.** 사용자는 불평을 했는데
# 시스템은 명령으로 읽었고, 그 위에서 open_app이 «앞으로 가져왔습니다»라고 거짓
# 보고까지 했다. 결함 두 개가 겹쳐 «뻥카»가 됐다.
#
# 「설정」+「실행」이 문장 **어디에든** 있으면 intent가 잡히는데, 어미 검사는
# 명령형으로 끝나는 어절이 없어 못 걸렀다. → **커버율**로 막는다.
print("=== 🚨 명령어를 스쳐 지나가는 긴 문장은 캐시를 타지 않는다 ===")
for _t in [
    "아니 그 컴퓨터 설정 그거부터 열어야 되는 거 아니니",
    "아니 설정 창은 애초에 실행 중이지 않았는데 뭘 가져와 너 왜 나한테 뻥카 치냐",
    "아까 메모장 열어달라고 했는데 왜 안 열렸는지 그 이유가 궁금하다고",
]:
    _hit = cache.find(_t)
    check(f"막힌다: {_t[:24]}…", _hit is None or cache.has_uncovered_command(_t))

# ⚠️ 반대쪽도 반드시 본다 — 가드가 넓으면 **정상 명령이 무더기로 캐시를 못 탄다.**
#    (command_cache.py 주석이 명시적으로 경고하는 실패 방향이다)
print("=== 짧고 곧은 명령은 그대로 캐시를 탄다 (가드가 넓지 않은지) ===")
for _t in ["설정 열어줘", "메모장 열어줘", "크롬 열어줘", "계산기 실행해줘",
           "스크린샷 찍어줘", "볼륨 올려줘", "음소거 해줘"]:
    _hit = cache.find(_t)
    check(f"통과: {_t}", bool(_hit) and not cache.has_uncovered_command(_t))

# ── 🔎 2026-09-12 — 「하나」 단독은 군말이다 (LAUNCH_CHECKLIST §7 「결함 B」) ──────
#
# *"메모장 하나 띄워봐"* 가 **캐시를 통째로 포기**하고 있었다. 온라인에서는 LLM이
# 받아 주니 안 보이지만, **오프라인에서는 «되는 명령이 안 되는» 것**으로 보인다 —
# 제안조차 안 떴다.
#
# 🔑 「하나」가 «한 개 더»를 뜻하려면 **「더」나 「또」가 붙는다.** 그 둘은 이미
#    `_RESIDUAL_MODIFIERS`에 있으므로, 「하나」는 **혼자서 잡아야 할 것을 잡은 적이
#    없고 오탐만 만들고 있었다.**
#
# ⚠️ **이 테스트는 양방향이다.** 아래 두 블록 중 하나만 지우면 회귀를 놓친다 —
#    넓히면 «새로 열기»가 깨지고(BL-15), 좁히면 정상 명령이 조용히 막힌다.
print("=== 「하나」 단독은 캐시를 탄다 (결함 B) ===")
for _t in ["메모장 하나 띄워봐", "계산기 하나 켜줘", "메모장 하나 열어줘"]:
    check(f"통과: {_t}", bool(cache.find(_t)) and not cache.has_uncovered_command(_t))

print("=== 「하나」에 「더」·「또」가 붙으면 여전히 막힌다 (BL-15를 안 깬다) ===")
for _t in ["메모장 하나 더 열어줘", "메모장 또 하나 열어줘", "메모장 더 열어줘",
           "새로 메모장 열어줘", "메모장 추가로 열어줘", "메모장 따로 열어줘"]:
    check(f"막힌다: {_t}", cache.has_uncovered_command(_t))
