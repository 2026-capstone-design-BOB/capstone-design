...
🚨 **이 주소만 뿌린다.** 프로젝트의 프로덕션 도메인이라 **재배포하면 자동으로 최신을 가리킨다.**
`vercel deploy` 가 찍어 주는 `pluiz-collect-<해시>-....vercel.app` 은 **그 배포에 고정**이라,
카톡에 뿌린 뒤 고치면 사람들은 옛 버전을 쓴다. (그래서 수동 별칭도 일부러 안 남겼다.)

✅ 배포 보호(Vercel Authentication)는 **2026-09-16에 해제됐다** — 모르는 사람이 열어야 하니까.

## 아직 안 해 본 것

- ⚠️ **업로드 경로 전체를 사람이 한 번도 안 돌려 봤다.** 마이크 권한 → 녹음 → «보내기» →
  `npm run pull` 까지는 **실기에서만 갈린다.** 남에게 뿌리기 전에 본인이 한 번 끝까지 할 것.
- 확인된 것: 페이지·번들 200, `/api/meta` 가 동의 없는 요청을 400 으로 막고
  동의 있으면 경로를 준다(**한글 이름 포함** — `pluiz_오다영_20260916192000`).
  테스트로 만든 blob 은 지웠다.

> 📌 **한글 이름은 멀쩡하다.** 점검 중에 `curl` 로 보냈을 때 깨져 보였는데,
> Git Bash 가 인자를 cp949 로 바꾼 것이었다. **UTF-8 바이트로 보내면 정상**이고
> 브라우저 `fetch` 는 UTF-8 로 보낸다. 다음에 같은 걸 의심하게 되면
> **`--data-binary @파일`** 로 재현할 것 — 셸을 거치지 않는다.
