...
| **계획 수립** | `planner` 노드 — 복합 명령을 **최대 2단계**로 나눠 순서대로 실행하고, **못 한 단계를 정직하게 말한다**(2026-09-05). replan 없음(LLM 왕복 턴당 1회). ⚠️ **기본 꺼짐** — `.env` `PLAN_ENABLED=true`로 켠다. mock 28건 통과, **실기 미검증** → [design/M3_계획수립노드.md](design/M3_계획수립노드.md) |
...
| **테스트** | mock **29파일 745개** + 라이브 3스위트. CI는 그중 28파일 713개 자동 실행 (`test_dependencies` 32개는 로컬 전용) — **이 숫자의 출처는 이 표 하나다** |
...
| [M3_계획수립노드.md](design/M3_계획수립노드.md) | 복합 명령을 단계로 쪼개 실행하는 `planner` 노드. **설계 완료 · 구현 미착수**. `recursion_limit: 10`이 이미 도구 3회를 상한으로 묶고 있다는 발견 포함 |
