...
⚠️ 다만 **좌표 클릭은 여전히 실기 미검증**이고, 감시는 **항상 켜지지는 않는다**
→ [TASKS.md](TASKS.md) ① · [BACKLOG BL-19](BACKLOG.md).
**2026-09-05에 10월 「Plan-and-Execute」를 설계했다** — [design/M3_계획수립노드.md](design/M3_계획수립노드.md).
**코드 변경은 없다(설계만).** 그 과정에서 `recursion_limit: 10`이 이미 순차 도구 호출을
**3회로 묶고 있다**는 것과 **BL-20**을 발견했다. → [ROADMAP.md](ROADMAP.md)
...
| **화면 감시** | `watch_screen` — *"오류 뜨면 알려줘"*. 5초마다 **로컬** 픽셀 비교로 거르고 변화가 있을 때만 Vision. 최대 10분·화면 20장에서 자동 종료하고 **어떤 이유로 멈추든 알린다**(2026-09-03). 승인 대신 **고지**(요약되지 않게 원문 그대로 나간다). ⚠️ **BL-19 — 항상 켜지지는 않는다.** 모델이 도구를 안 부르는 구간이 있다. 실패하면 *"시작하지 못했어요"* 라고 **정직하게** 답한다(2026-09-04). 알림 도달은 여전히 미검증 → [TASKS.md](TASKS.md) ① |
| **계획 수립** | `planner` 노드 — 복합 명령을 **최대 2단계**로 나눠 순서대로 실행하고, **못 한 단계를 정직하게 말한다**(2026-09-05). replan 없음(LLM 왕복 턴당 1회). ⚠️ **기본 꺼짐** — `.env` `PLAN_ENABLED=true`로 켠다. mock 37건 통과. 2026-09-07 첫 라이브에서 **건너뛴 단계를 완료로 세던 결함**(BL-21)을 고쳤고 **재확인 대기** → [M3](design/M3_계획수립노드.md) · [M3-1](design/M3-1_단계완료판정.md) |
...
| **테스트** | mock **30파일 783개** + 라이브 3스위트. CI는 그중 29파일 751개 자동 실행 (`test_dependencies` 32개는 로컬 전용) — **이 숫자의 출처는 이 표 하나다** |
| **라이브 실측** | `regression`에 **Vision 라이브 V-01~V-05 추가**(2026-09-04). **V-01~V-04 통과** — 실제 캡처 → Gemini 왕복 → 좌표 환산이 처음으로 자동 검증됐다. **V-05는 BL-19 때문에 구간에 따라 FAIL한다**(회귀 아님 — 실패해도 거짓말은 안 한다).<br>2026-09-03 기준 `commands` 40P/0F/15MANUAL · `sprint1_2` 55/55 → [DEVLOG](DEVLOG.md).<br>⚠️ **미검증(전부 사용자가 봐야 함)**: 마이크 · UI 시각 확인 · 화면 검증 · **좌표 클릭** · **감시 알림 도달** → [TASKS.md](TASKS.md) ① |
| **CI** | GitHub Actions 3잡 — mock · 문서링크 · 비밀정보 가드 |
| **브랜치** | `main` = `develop` = 최신. 작업은 `feature/byeonsoyun` |
| **[즉시/위험]** | **BL-19 부분 해결** — 거짓 보고는 막았으나 감시가 항상 켜지지는 않는다.<br>**BL-20 신규**(2026-09-05, 미수정) — 위험 도구 거부가 같은 배치의 **안전한 호출까지** 조용히 취소한다. 실기 관측은 없지만 **병렬 실행의 선행 수정** → [BACKLOG](BACKLOG.md) |
