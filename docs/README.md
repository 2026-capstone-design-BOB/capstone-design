...
⚠️ 남은 미검증은 **웨이크워드(마이크)** 와 **BL-13 설정 화면**이다 → [TASKS.md](TASKS.md) ①
...
| **테스트** | mock **31파일 816개** + 라이브 3스위트. CI는 그중 30파일 782개 자동 실행 (`test_dependencies` **34개**는 로컬 전용 — 2026-09-08에 lock 대조 2건 추가) — **이 숫자의 출처는 이 표 하나다** |
...
| [M2_웨이크워드_전용모델.md](design/M2_웨이크워드_전용모델.md) | Whisper로 웨이크워드가 안 되는 이유(실측) + 전용 모델 학습 설계. **진행 중** |
