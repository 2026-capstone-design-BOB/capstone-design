...
| **[즉시/위험]** | **BL-26 해결**(2026-09-09) — «안 한 걸 했다고 말한 것». 뿌리는 **유령 창**이었다(정지된 UWP의 cloaked 창을 «있는 창»으로 셌다). ⚠️ 교훈: **«확인한다»는 절반이고 «무엇을 창으로 볼 것인가»가 나머지 절반**이다 — 1차 수정이 같은 유령에 속아 통과했다.<br>**BL-25 해결**(2026-09-09) — 음성이 첫 번째만 되던 것(레코더 두 개).<br>**BL-20 해결**(2026-09-09) — 위험 도구 거부가 같은 배치의 **안전한 호출까지** 조용히 취소하던 것. 안전한 호출을 **새 id로 재발행**해 살린다 → [ADR](design/BL-20_거부후_안전호출.md). ⚠️ **라이브 미검증**(위험+안전 배치가 실기에서 드물다).<br>**BL-24 신규**(미수정) — 위험 호출이 **둘**이면 질문은 하나만 나가고 승인 시 **둘 다** 실행된다. BL-20의 재발행 기계가 그대로 답이 된다.<br>**BL-27 해결**(2026-09-10) — 캐시가 **승인 응답 「그래」를 `close_app`으로 학습**했던 것(승인 맥락이 아닐 때 「그래」라고 하면 시킨 적 없는 앱 종료가 실행됐다 — 캐시는 HITL을 안 거친다). **발화** 쪽 학습 자격(L1·L2·L3)과 부정·대조 게이트를 넣고 이미 박힌 4개를 **선별 제거**했다(동적 7 → 3). ⚠️ 교훈: **ADR에 쓴 필터가 틀린 걸 구현 중에 쟀다** — «entity와 action 둘 다»는 시드 9개를 막았다 → [M5 §3-3 정정](design/M5_임베딩_캐시.md)<br>**BL-19 부분 해결** — 거짓 보고는 막았고 재시도를 `tool_choice` 강제로(2026-09-07). 호출률 자체는 모델 쪽이라 그대로.<br>**BL-21 해결**(2026-09-07) — 계획이 건너뛴 단계를 완료로 세고 **캐시에 학습**되던 것 → [BACKLOG](BACKLOG.md) |
