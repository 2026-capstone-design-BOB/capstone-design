...
| **테스트** | mock **32파일 847개** + 라이브 3스위트. CI는 그중 31파일 813개 자동 실행 (`test_dependencies` **34개**는 로컬 전용 — 2026-09-08에 lock 대조 2건 추가) — **이 숫자의 출처는 이 표 하나다** |
...
| **[즉시/위험]** | **BL-20**(미수정) — 위험 도구 거부가 같은 배치의 **안전한 호출까지** 조용히 취소한다. 실기 관측은 없지만 **병렬 실행의 선행 수정**이고, 계획이 기본 ON이 되면서 이 경로를 더 자주 밟는다.<br>**BL-19 부분 해결** — 거짓 보고는 막았고 재시도를 `tool_choice` 강제로(2026-09-07). 호출률 자체는 모델 쪽이라 그대로.<br>**BL-21 해결**(2026-09-07) — 계획이 건너뛴 단계를 완료로 세고 **캐시에 학습**되던 것 → [BACKLOG](BACKLOG.md) |
