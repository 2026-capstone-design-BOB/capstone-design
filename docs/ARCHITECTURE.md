...
> ⚠️ **(2026-09-07에 라이브 확인됨 — 아래 문단은 그 이전 기록이다.)** mock 37건은 통과했지만 `PLAN_ENABLED=true`로 실제 명령을 돌려 본
...
### `요청=` 과 `실행=` 은 다른 것이다 (BL-52, 2026-09-12)

`턴 완료` 줄에는 도구 칸이 **둘**이다. 같은 말이 아니다.

| 칸 | 무엇을 세나 | 근거 |
|---|---|---|
| `요청=` | LLM이 **달라고 한** 도구 | 이번 턴 `AIMessage.tool_calls` |
| `실행=` | `ToolNode`가 **실제로 돌린** 도구 | 이번 턴 `ToolMessage`(이름이 붙은 것) |
...
> 🚨 **2026-09-12까지 이 칸은 `도구=` 하나였고, 그 값은 「요청」이었다.**
> 위 줄이 `도구=['delete_file','delete_file']` 로만 찍혀서
> **아무것도 안 지운 턴이 «delete_file 두 개»로 읽혔다** — 실제로 그날
> *«계획+승인이면 삭제가 두 번 실행된다»* 고 오판했다가, 같은 줄이 거부 턴에도
> 찍히는 걸 보고 정정했다. (둘로 세어지는 건 [BL-24](BACKLOG.md)의
> «원본 마감 + 새 id 재발행» 때문이다)
>
...
호출을 마감하며 지어내는 ToolMessage(«취소했습니다»·«보류됐습니다»)는 **이름이 없다.**
문구를 비교하지 않는다.

⚠️ **캐시 히트는 `실행=['캐시']`** 다. 도구를 그래프 **밖에서** 돌리므로
(절대규칙 2) ToolMessage가 없는데, 그대로 두면 `실행=없음`이 되어 «아무것도 안 했다»로
읽힌다. 이름은 바로 위 `[FastPath] [캐시 실행]` 줄에 있다.

⚠️ **옛 이름 `도구=` 도 계속 읽힌다** — 09-12 이전 로그가 이미 쌓여 있다.
그 구간은 `실행=` 칸이 없어 **요청을 실행으로 치므로 승인이 낀 턴만큼 부풀어 있고**,
`analyze_tool_usage.py`가 리포트에 그렇게 적는다.

### 도구를 안 부른 턴의 «사유» (BL-28, 2026-09-10)

도구를 **하나도 안 부른** 턴에는 `| 사유=…` 가 붙는다:
`캐시` · `차단` · `승인거부` · **`못함`** · `잡담`.

```
턴 완료 | 입력='인쇄 버튼 눌러줘' | 요청=없음 | 실행=없음 | 응답 40자 | 사유=못함 | 소요 3.10s | …
```

> **왜 필요했나:** 응답이 «13자»로만 남아 *"못 했어요"* 와 *"이미 열려 있어요"* 를
> 사후에 못 갈랐다. 그래서 [도구 사용 실측](research/2026-09_도구사용_실측.md) ②
> («도구가 없어서 실패한 턴»)에 **답할 수 없었다.** 이제 «못함»만 세면 된다.
>
> ⚠️ **응답 전문은 남기지 않는다.** 분류는 프로세스 안에서 하고 로그에는 한 단어만
> 나간다 — 개인정보가 로그로 새면 안 된다.
> ⚠️ 도구가 하나라도 돌았으면 **안 붙는다.** 그 턴은 «안 부른 턴»이 아니다.

그리고 캐시가 그래프 **밖에서** 돌린 도구는 `[FastPath] [캐시 실행] 패턴=… | 도구=[…]`
로 남는다. 이 줄이 없던 시절에는 **볼륨·밝기·스크린샷·시간·배터리가
«한 번도 안 쓰인 도구»로 집계됐다** — 실제로는 캐시가 돌리고 있었는데도.

⚠️ 이 줄들의 **형식**은 `tests/test_log_format.py`가 고정한다. 생산자
(`core/graph_agent.py`)와 소비자(`scripts/analyze_tool_usage.py`)가 다른 파일이라
한쪽만 고치면 **오류 없이 «0턴»이라고 말한다.**

### 턴 계측 — `턴 완료` 줄이 성능 데이터다 (2026-09-08)

```
턴 완료 | 입력='메모장 켜줘'   | 요청=없음 | 실행=['캐시'] | 응답 11자 | 소요 0.21s | LLM 0회(캐시) | 토큰 0
턴 완료 | 입력='오늘 날씨 어때' | 요청=없음 | 실행=없음 | 응답 24자 | 소요 1.83s | LLM 1회 | 토큰 in=1240 out=56
```
...
> 🚨 **2026-09-10 실측: Stage 2는 사실상 발화하지 않는다.** 평가셋 32문장에서
> 임계를 넘은 것이 **0개**였다(의미가 같은 문장이 0.71~0.76에 몰려 있는데 임계는 0.80).
> **캐시의 재현율은 통째로 손으로 쓴 entity/action 사전이 낸다.**
>
> ⚠️ 이 자리의 옛 주석(*"`_similarity()`만 임베딩 코사인으로 교체하면 된다"*)은
> **불충분한 것으로 판명됐다.** 모델 3종·중심화·검증기 격자를 다 재도
> **오매칭 0인 임계가 존재하지 않았다** — 안전하게 쓰면 현행보다 못하다.
> 그래서 임베딩 교체는 **⏸️ 보류**다 → [M5 §6-3](design/M5_임베딩_캐시.md)

**Stage 0 — 부정·대조 게이트**(BL-27, 2026-09-10): 「말고 · 대신 · 아니라 · 빼고」가
어절로 있으면 **두 단계 모두 건너뛰고 LLM으로 보낸다.** 캐시는 문장에서 처음 걸린
entity 하나만 집어서 「메모장 **말고** 계산기 열어줘」에 **메모장을 열었다.**
⚠️ 이건 임베딩으로 바꿔도 안 고쳐진다(문장 임베딩은 부정에 약하다) — **고치지 않고 피한다.**

**Stage 0-b — 지시대명사 게이트**(BL-50, 2026-09-12): 「그거 · 이거 · 저거 · 거기 ·
여기 · 걔 …」가 **어절 머리**에 있으면 역시 두 단계 모두 건너뛰고 LLM으로 보낸다.
1차 리허설에서 `'그거 꺼줘' → close_app(메모장)` 이 학습돼 **크롬을 보며
「그거 꺼줘」 하면 메모장이 꺼지는** 상태였다. 「그거」는 **직전 턴에만 의미가 있는 말**인데
캐시는 맥락을 안 본다 — 맥락을 가진 쪽은 LLM뿐이라, 막아서 가는 곳이 옳은 곳이다.
...
- 학습 금지 **①도구 쪽**(`_is_learnable`): 자유 파라미터(폴더명·검색어·`set_volume`
  숫자·`type_text`), 위험 명령, 실패한 명령, 보안 차단 입력
...
- 학습 금지 **③짝 쪽**(`query_conflict`, BL-60 2026-09-19): **묻는 말**을 **조작
  ...
  > ⚠️ L1은 «둘 다»가 아니라 **«둘 중 하나»** 다 — 둘 다로 하면 시드 37개 중 9개가 막힌다
  > (`창 최대화해줘`에는 entity가 없다). 실측으로 정정했다 → [M5 §3-3](design/M5_임베딩_캐시.md)
  > 🚨 **L4는 L1~L3이 못 막는 축이다**(BL-50, 1차 리허설). `'그거 꺼줘'`는 「꺼」가
  > action이라 L1을, 5글자라 L2를, 대조가 없어 L3을 **전부 통과했다.**
  > ⚠️ **한 번 지우는 걸로는 안 된다** — 대본을 돌 때마다 다시 박힌다. 게이트라야 한다.
