...
| 어디서 | 결과 |
|---|---|
| **cmd / Anaconda Prompt** | ✅ **전부 통과** — 이 환경이 맞는 숫자를 낸다 |
| Git Bash | ❌ **1파일이 통째로 실패한다**(6건 모자라게 나온다) — **프로젝트 회귀가 아니다** |
| PowerShell | ⚠️ `conda activate`가 안 먹어 **base 파이썬**으로 돈다 → [BL-59](BACKLOG.md) |

🔑 **실제 실행 경로(`launch.bat` = cmd)는 영향이 없다.** 앱은 멀쩡하다.
📌 **여기에 합계를 적지 않는다** — 2026-09-18에 같은 합계가 네 문서에 박혀 있다가
   **하루에 넷이 동시에 어긋났다.** 합계의 출처는 [README 상태표](README.md) 하나다.
   (위 차이는 2026-09-17 실측 당시 49파일 1738 기준이었고, **차이의 크기는 그대로다**)
...
> 🔑 **왜 체크리스트에 넣었나 (2026-09-22)** — 교수님 피드백이 *"기능을 만든 건 알겠는데
> 어필을 못 한다"* 였고, 원인은 **재료가 없어서가 아니라 개발 문서에만 있어서**였다.
> 팀원이 매번 물어봐야 하면 결국 안 물어보고 낡은 숫자를 쓴다.
> **프롬프트는 확률을 올릴 뿐이고 보장하는 건 구조다** — 그래서 절차에 박는다.

| 걸리는 것 | 고칠 곳 |
|---|---|
| **① 발표에 쓸 숫자가 바뀌었다** (도구 개수·테스트 수·측정값) | [teamwork/10_개발_현황.md](teamwork/10_개발_현황.md) §1 |
| **② 팀원의 작업 내용·순서·마감에 영향이 있다** | 같은 문서 §2에 날짜 블록 추가 — **「당신 작업에 무슨 뜻인가」 칸을 반드시 채운다** |
| **③ 새 기능이 생겼거나 «말하는 방법»이 바뀌었다** (새 단서·하면 안 되는 말) | [teamwork/11_어필_재료.md](teamwork/11_어필_재료.md) |

- [ ] 교수님 피드백을 받았으면 [teamwork/피드백_기록.md](teamwork/피드백_기록.md)에 **원문 그대로**
- [ ] 개발 용어가 새로 나왔으면 [teamwork/12_용어_사전.md](teamwork/12_용어_사전.md)

⚠️ **개발 내부 사정은 `teamwork/`에 적지 않는다.** 그건 DEVLOG다.
**팀원의 작업에 영향이 있는 것만** 넘긴다 — 경계를 지켜야 그 폴더가 계속 읽힌다.
...
#### 🚨 사용자가 «푸시하자»고 하면 — **나가기 전에 이 넷을 본다**

**푸시는 되돌려도 기록에 남는다. public 저장소다.**

- [ ] **CI 가드를 로컬에서 먼저 돌린다** (`.github/workflows/tests.yml`의 `secret-guard`)
      — ① `.env` 추적 여부 ② 실제 API 키 패턴 ③ `.pyc`
- [ ] 🆕 **개인정보** — **나갈 diff에 «남의 이름·학번·연락처»가 새로 들어갔는지 본다**
      ```bash
      git diff origin/<브랜치>..HEAD | grep "^+" | grep -E "이름1|이름2|…"
      ```
      🚨 **2026-09-18에 실제로 걸렸다** — 녹음 참여자 3명의 **개인별 인식률이 실명으로**
      들어가 있었다. **이름은 이미 저장소에 있었지만 «이름 + 성능 수치»는 새것이었다.**
      → **화자 A·B·C로 바꾸고 본인 것만 실명으로 뒀다.** 대응표는 저장소에 두지 않는다.
      📌 **학번이 올라가 있던 2026-09-16 건과 같은 성질**이다
- [ ] **큰 파일** — diff에 수백 KB 이상이 섞이지 않았는지
- [ ] **푸시 뒤 확인** — `git rev-parse HEAD` 와 `origin/<브랜치>`가 같은지, 미푸시 0인지

> 🔑 **점검 목록에 «개인정보»가 없었던 것이 2026-09-18의 교훈이다.**
> 비밀정보(키)는 CI가 보는데, **남의 개인정보는 아무도 안 보고 있었다.**
