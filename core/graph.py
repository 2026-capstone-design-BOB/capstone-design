...
# 승인 대기 중에 «명령»으로 인정할 어절 수 상한. (BL-53)
#
# 🚨 **왜 상한이 필요한가 — 2026-09-14 2차 리허설.** 사용자가 승인 질문에
#    "그래"라고 답했는데 STT가 **13어절 49자**를 올려 보냈고, 그게 명령으로
#    읽혀 **삭제가 조용히 취소됐다.**
#      '크롬 엣지 카카오톡 탐색기 바탕화면 다운로드 문서 폴더 파일 스크린샷 최대화 최소화 검색'
#
# 🔑 **뚫린 이유가 우연이 아니다.** 저 문자열은 `services/stt.py`의
#    `_WHISPER_PROMPT`를 Whisper가 그대로 되뱉은 것이고, 그 프롬프트의 꼬리가
#    "… 최대화 최소화 **검색** 실행 종료 …"다. 그리고 `_COMMAND_TAIL_RE`에도
#    `검색|실행|종료`가 있다 — **둘 다 «PC 제어 어휘»에서 뽑은 목록이라 같이
#    무너진다.** 진짜 그물은 stt.py의 `is_prompt_echo`가 친다(1층).
#    여기는 **그걸 뚫은 다른 환각까지 받는 마지막 방어선**이다.
#
# ⚠️ 상한을 넘으면 `unclear`가 되어 **재질문**한다(`_MAX_CONFIRM_ASKS`).
#    두 번 다 못 알아들으면 «취소했습니다»로 끝난다 — 조용히 잡담으로 새는
#    오늘보다 낫다. **사용자에게 무슨 일이 일어났는지 말하는 쪽이다.**
#
# 8로 둔 근거: 시연 대본에서 가장 긴 명령이 "메모장 열고 b.txt도 지워줘"(3어절)이고,
# 현실적으로 긴 복합 명령도 5~6어절이다("메모장 열고 계산기도 열고 볼륨도 올려줘").
# 13어절 나열과는 사이가 넉넉하다.
...
def _looks_like_command(tokens: list[str], text: str) -> bool:
    ...
    if len(tokens) < 2:
        return False
    # 승인 대기 중의 답은 짧다. 긴 나열은 사람의 명령이 아니라 STT 사고다. (BL-53)
...
def build_system_prompt() -> str:
    ...
    return (
        ...
        #    → 한 턴에 모아 부르게 해야 BL-24가 비로소 작동한다.
        #
        #    왜 이게 답답함을 넘어 안전 문제인가: 같은 질문이 N번 반복되면 사용자는
        #    **읽지 않고 「그래」를 말하게 된다.** 실기 로그에 그게 남아 있다 —
        #    17:23~17:24에 '오'·'음'·'그래라는 뜻이야'로 대충 답하고 있었다.
        #    **승인 피로는 승인을 무력화시킨다.**
        #
        #    ⚠️ **해야 할 일을 먼저, 제약을 뒤에** 둔다 (BL-19의 교훈 — 금지문을 앞에
        #    두면 모델이 «아무것도 안 하는 쪽»으로 기운다).
        ...
        #    🚨 **`delete_file`은 일부러 안 푼다** — 삭제에 탐색을 붙이면
        #    *"보고서.docx 지워줘"*가 사용자가 생각한 적 없는 파일을 지운다
        #    (`_locate_for_open`의 «의도된 비대칭»). 그래서 **삭제 경로는 모델이
...
- **같은 동작을 여러 대상에 하는 것은 나누지 않습니다.** 그건 한 단계입니다.
예) 입력: 메모장 열고 크롬 닫아줘
1. 메모장 열기
2. 크롬 닫기
예) 입력: a.txt랑 b.txt 지워줘
(아무것도 출력하지 않음 — 같은 동작이라 한 단계다)"""

_PLAN_STEP_RE = re.compile(r'^\s*\d+\s*[.)]\s*(.+?)\s*$')

# ── 같은 동작은 쪼개지 않는다 (BL-30) ────────────────────────────
#
# 프롬프트로만 부탁하면 «확률»이고, 이 저장소는 그 교훈을 여러 번 샀다(BL-19).
# 그래서 추출 단계에서 **구조로** 막는다.
#
# 왜 쪼개면 안 되나 — 2026-09-10 실기:
#   ① 승인이 **두 번** 뜬다. 승인 피로가 승인 절차 자체를 무력화한다(BL-24 ADR §4)
#   ② 2단계가 위험 도구면 LLM이 **그 단계에서 도구를 안 부르는** 일이 있었다
#      → M3-1이 "못 했어요"라고 정직하게 보고하지만, 사용자는 두 번 말해야 한다
#   ③ 쪼개지 않으면 BL-24의 «전부를 이름으로 부르는 질문»이 그대로 받는다 —
#      *"'a.txt' 파일과 'b.txt' 파일 2개를 정말 삭제할까요?"*
#
# ⚠️ **동작 어휘를 넓게 잡지 않는다.** 넓히면 «메모장 열고 계산기 열어줘»처럼
#   정말 나눠야 하는 것까지 한 단계가 된다 — 그건 M3가 존재하는 이유다.
#   여기 있는 것은 **되돌릴 수 없어 승인이 붙는 동작**과 그 이웃뿐이다.
_STEP_ACTIONS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("삭제", ("지우", "삭제", "휴지통", "없애", "치우")),
    ("열기", ("열기", "열어", "실행", "켜기", "켜줘", "띄우", "띄워")),
    ("닫기", ("닫기", "닫아", "종료", "끄기", "꺼줘")),
    ("만들기", ("만들", "생성", "새로만들")),
)
...
def _step_action(step: Any) -> str:
    """단계 문장이 가리키는 **동작**. 모르면 빈 문자열.

    표현이 달라도 같은 동작이면 같은 값이 나와야 한다 —
    `'a.txt 지우기'` 와 `'b.txt 삭제'` 는 둘 다 `"삭제"` 다.
    """
    t = _plan_norm(step)
    for name, words in _STEP_ACTIONS:
        if any(w in t for w in words):
            return name
    return ""


#: 묶을 동작 — **되돌릴 수 없어 승인이 붙는 것만.**
#:
#: ⚠️ 처음엔 «같은 동작이면 무엇이든» 묶었는데, 그러면 `'메모장 열기' / '계산기 열기'`
#:   까지 한 단계가 된다. 열기는 **쪼개도 피해가 없고**(승인이 없다) 쪼개야
#:   M3-1이 *"계산기는 못 했어요"* 를 말할 수 있다 — 그게 M3의 존재 이유다.
#:   실제 피해(승인 두 번 · 위험 단계 미실행)는 **승인이 붙는 동작에서만** 난다.
_BATCHABLE_ACTIONS = frozenset({"삭제"})


def _same_action_on_many(steps: list[str]) -> bool:
    """«같은 동작 · 다른 대상»이고, 그 동작이 **묶어서 물어야 하는 것**인가.

    ⚠️ 동작을 **모르면 False**다 — 모르는 건 나누게 둔다. 여기서 True를 남발하면
      정말 나눠야 할 복합 명령이 한 단계로 뭉쳐 M3가 무력해진다.
    """
    actions = [_step_action(s) for s in steps]
    if not actions[0] or len(set(actions)) != 1:
        return False
    return actions[0] in _BATCHABLE_ACTIONS
...
def parse_plan(raw: Any, original: str = "") -> list[str]:
    ...
    버리는 경우(전부 `[]`):
      ...
      - **같은 동작을 여러 대상에 한다** (BL-30) — `'a.txt 지우기' / 'b.txt 지우기'`.
        쪼개면 승인이 두 번 뜨고, 2단계가 위험 도구일 때 실행이 새는 일이 있었다.
        버리면 LLM이 한 배치에 담고 **BL-24가 «2개를 정말 삭제할까요?»로 한 번에** 묻는다.
    """
    if raw is None:
        return []
    ...
    if original and _plan_norm(original) in norms:
        return []
    if _same_action_on_many(steps):
        _log.info("[Plan] 같은 동작이라 나누지 않는다 (BL-30) | 단계=%r", steps)
        return []
    return steps
...
def build_pluiz_graph(
    *,
    ...
    def hitl(state: PluizState) -> dict:
        ...
        # 🔬 **BL-40 계측.** «한 번 묻고 N개를 처리했는가»를 로그로 셀 수 있게 남긴다.
        #   이게 없으면 프롬프트 유도(A안)가 먹었는지 **다음 실기에서도 알 수 없다** —
        #   2026-09-11에는 `승인 대기`가 두 번 찍힌 것을 보고 «쪼개졌다»를 **추론**해야 했다.
        #   이제 `위험 1개`가 연달아 두 번 찍히면 쪼개진 것이고, `위험 2개`가 한 번
        #   찍히면 A안이 먹은 것이다. 11월 «human 개입» 지표의 분모도 여기서 나온다.
        _log.info("승인 질문 | 위험 %d개 | 안전 %d개 | 없는대상 %d개 | 대상=%s",
                  len(askable), len(safe_calls), len(missing),
                  [_target_name(c) for c in askable])
        verdict = "unclear"
        answer: Any = ""
